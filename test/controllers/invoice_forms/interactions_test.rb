...
class InvoiceForms::InteractionsTest < ActionDispatch::IntegrationTest
  setup do
  ...
  test "calculations and row changes never persist an invoice or line" do
    ...
    assert_no_difference [ "Invoice.count", "InvoiceLine.count" ] do
      post invoice_forms_calculation_path, params: attributes, headers: @headers
      assert_response :success
      assert_select "turbo-stream[target='invoice_total'] template", text: "NGN 350.00"
      get new_invoice_forms_line_path, headers: @headers
      assert_response :success
      assert_select "turbo-stream[action='append'][target='invoice_lines']"
      delete invoice_forms_line_path("0"), params: attributes, headers: @headers
      assert_response :success
  ...
  test "customer selection only renders details for the unsaved form" do
    ...
    assert_no_difference "Invoice.count" do
      get invoice_forms_customer_path(contact_id: customer.id), headers: @headers
    end
    assert_response :success
    ...
    assert_select "input[type='hidden'][name='invoice[contact_id]']", count: 0
  end
